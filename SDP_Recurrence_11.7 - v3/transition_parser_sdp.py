...
class TransitionParser(Model):
    ...
    def forward(self,
        ...
        embedded_pos_tag = self.pos_tag_field_embedder(pos_tags)
        ...
        # embedded_text_input.shape = torch.Size([30, 29, 100])
        # token_characters.shape = torch.Size([30, 29, 100])
        # embedded_pos_tag.shape = torch.Size([30, 30, 50])
        ...
        # 评价模型
        if "arc_indices" in metadata[0].keys():
